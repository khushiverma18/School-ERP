// 1. Import motion from framer-motion for animations
import { motion } from "framer-motion";
import { Card, CardContent } from "@/components/ui/card";
import { GraduationCap, Building, Users, Award, Globe } from "lucide-react";

const institutions = [
  // ... (Your institutions array remains unchanged)
  {
    icon: GraduationCap,
    title: "Schools (K-12)",
  ...
  return (
    ...
    <section className="relative overflow-hidden bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 py-24 sm:py-32">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* 4. Use motion.div for the grid and apply containerVariants */}
        <motion.div
        ...
        >
          {institutions.map((institution, index) => (
            // 5. Card container with motion.div and itemVariants for staggered animation
            <motion.div
              key={index}
            ...
            >
              {/* This inner div creates the glassmorphism effect, now slightly enhanced for your background */}
              <div className="relative h-full rounded-[15px] bg-slate-900/85 p-8 backdrop-blur-lg transition-colors duration-300 group-hover:bg-slate-900/90">
                <div className="mb-6 flex justify-center">
                  {/* Icon with a subtle glow and hover effect */}
                ...
                {/* 6. Animated gradient glow effect on hover, now with dynamic colors */}
                <div className="pointer-events-none absolute inset-0 -z-10 rounded-2xl opacity-0 transition-opacity duration-300 group-hover:opacity-100"
                  style={{
                    background: "radial-gradient(400px circle at 50% 0%, rgba(56, 189, 248, 0.15), transparent 80%)" // Using the sky/cyan color from the title
                  }}
                />
              </div>
            </motion.div>
